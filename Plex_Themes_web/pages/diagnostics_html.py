...
class Diagnostics_HTML():
    ...
    def get(self, *args):
            ...
            if route_group not in ["pages", "brython", "files", "tool_pages"]:
                ...
                for route_item in list(session["route_list"][route_group].keys()):
                    if not session["route_list"][route_group][route_item]["name"].startswith("page_"):
                        ...
                        button_link = button_link.replace("<devicekey>", self.tvtunes.config.dict["main"]["uuid"])
        ...
        return render_template('diagnostics.html', request=request, session=session, tvtunes=self.tvtunes, button_dict=button_dict, route_group=route_group, route_groups=route_groups, list=list)
