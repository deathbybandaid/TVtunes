...
from TVtunes.tools import isint, closest_int_from_list, is_jsonable
...
class Logger():
    """
    The logging System for TVtunes.
    """
    ...
    def __init__(self, settings):
        ...
        logging_config = {
            ...
            'loggers': {
                # all purpose, TVtunes root logger
                'TVtunes': {
            ...
            'handlers': {
                ...
                'logfile': {
                    ...
                    'filename': os.path.join(
                        self.config.internal["paths"]["logs_dir"], '.TVtunes.log'),
                    'when': 'midnight',
                    'formatter': 'TVtunes',
                },
                # Memory Logging
        ...
        self.logger = logging.getLogger('TVtunes')
    ...
    def levelno(self):
        ...
        if isint(self.config.dict["logging"]["level"]):
            ...
            if self.config.dict["logging"]["level"].upper() not in levels:
                level = self.tvtunes.config.conf_default["logging"]["level"]["value"]
