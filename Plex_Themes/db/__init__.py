...
class TVtunesValues(BASE):
    __tablename__ = 'tvtunes_values'
...
class TVtunesdb(object):
    """
    The Database system for TVtunes.
    """
    ...
    def __init__(self, settings, logger):
        ...
        try:
            ...
            self.tvtunes.logger.error("OperationalError: Unable to connect to database.")
            raise
    ...
    def set_tvtunes_value(self, item, key, value, namespace='default'):
        """
        Set TVtunes value.
        """
        ...
        try:
            result = session.query(TVtunesValues) \
                .filter(TVtunesValues.item == item)\
                .filter(TVtunesValues.namespace == namespace)\
                .filter(TVtunesValues.key == key) \
                .one_or_none()
            ...
            # DNE - Insert
                ...
                new_itemvalue = TVtunesValues(item=item, namespace=namespace, key=key, value=value)
    ...
    def get_tvtunes_value(self, item, key, namespace='default'):
        """
        Get TVtunes value.
        """
    ...
    def delete_tvtunes_value(self, item, key, namespace='default'):
        """
        Delete TVtunes value.
        """
